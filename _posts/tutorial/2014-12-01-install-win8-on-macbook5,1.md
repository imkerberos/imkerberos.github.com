...
MacBook 467 单系统启动以后，总要白屏约 30 - 40 秒才开始引导 Windows。初步怀疑是我的两个 SSD 的分区格式是 `GPT` 而不是 `EFI` 格式的，导致 Mac 的 EFI 系统启后要模拟 `BIOS` 才造成的耗时，因为网上有些同学反映在双系统 (OS X + Windows) 下根本没有这个问题。

不折腾了，有精力的同学可以折腾一下使用 `EFI` 模式来安装 Windows。MacBook 是不是还长时间启动白屏。
